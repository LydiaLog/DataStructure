...
public class ListQueue implements Queue {
	...
	public void enqueue(Object x) {// 큐에 원소 x를 삽입한다
		...
		if (count == 0) {// 큐가 공백일 때
			...
			rear.link = newNode;// 빈칸1을 채우시오: rear의 link가 newNode를 가리킴
			rear = newNode;// 빈칸2를 채우시오: rear가 newNode를 가리킴
		}
		count++;
	}
	...
	public Object dequeue() {// 큐에서 원소를 삭제하고 반환
		if (count == 0)
			return null;
		Object item = front.data;// 빈칸3을 채우시오: Object 타입의 변수에 front의 data를 저장
		front = front.link;// 빈칸4를 채우시오: front가 다음 원소를 가리킴 
