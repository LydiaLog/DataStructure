...
public class postfix {
	public static String chanPostfix(String input) // 수식을 후위식으로 변환
	{
		...
		for (int i = 0; i < input.length(); i++) // 0부터 입력받은 문자열의 크기를 반복
		{
			...
			{
				postfix = postfix + c;// 빈칸 1을 채우시오: 문자 c가 숫자면 postfix에 숫자 추가
			...
			} else if (c == ')') // 문자 c가 ')'일 때
			{
				...
				{
					postfix +=  d;// 연산자를 postfix에 더함
					d = ((Character) (s.pop()));// 스택에서 '('이 나오기 전까지 pop함
					// 빈칸 2를 채우시오
				}
			}
			// c가 연산자일 때
	...
	public static int evalPostfix(String postfix) {
		...
		for (int i = 0; i < postfix.length(); ++i) // 문자열을 순회
		{
			...
			if (Character.getType(c) == 9) // 후위식에서 토큰이 숫자일 경우 스택에 푸시 getType은 타입 판별후 변환 
			{
				n = char2int(c);
				s.push(n);
